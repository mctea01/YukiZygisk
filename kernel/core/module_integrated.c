...
/*
 * Retained for source compatibility with core/control.c, which references the
 * bootstrap daemon-ready handshake. Integrated builds exclude core/bootstrap.o
 * and run no fail-closed guard; integrated control fds are installed with
 * bootstrap=false, so the YZ_IOCTL_DAEMON_READY handler rejects them before
 * reaching this stub. It exists purely to resolve the link.
 */
...
/*
 * Stable host API (declared in integrated.h) called by the host KernelSU's
 * KSU_IOCTL_YZ_INSTALL_FD dispatch handler. It hides the internal control-fd
 * signature from the host: integrated fds are always non-bootstrap.
 */
int yukizygisk_integrated_install_fd(void)
{
	return yukizygisk_control_install_fd(false);
}
